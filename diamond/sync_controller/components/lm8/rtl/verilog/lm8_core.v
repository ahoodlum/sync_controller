...
//      end
   ...
   generate
	...
	always @(local_ext_addr or page_ptr1 or import or export or lsp or ssp)
	  if (lsp || ssp || export || import)
	    ext_addr = {{8{1'b0}}, local_ext_addr};
	  else
            ext_addr = {page_ptr1, local_ext_addr};

      else if (EXT_AW <= 24)
	always @(local_ext_addr or page_ptr1 or page_ptr2 or import or export or lsp or ssp)
	  if (lsp || ssp || export || import)
	...
	always @(local_ext_addr or page_ptr1 or page_ptr2 or page_ptr3 or import or export or lsp or ssp)
	  if (lsp || ssp || export || import)
      ...
      begin
         if (!rst_n) begin
            ...
            wren_alu_rd <= #1 (add | addc | sub | subc | mov | andr |
			    ...
			    rcsr);
            wren_il_rd  <= #1 (import | importi | lsp | lspi);
         end
      end
   ...
   always@(import or importi or lsp or lspi or ext_mem_din or ext_io_din or din_rd1 or rcsr or dout_csr)
      begin
         if (lspi||lsp)
            din_rd = ext_mem_din;
